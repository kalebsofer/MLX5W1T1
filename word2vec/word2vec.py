...
def train_word2vec(filename, method="simple", vector_size=100, window=5, min_count=5, workers=4, epochs=5, negative=5, alpha=0.025, sample=1e-3, hs=0, cbow_mean=1):
    print(f"Now training Word2Vec with these hyperparameters: vector_size={vector_size}, window={window}, min_count={min_count}, workers={workers}, epochs={epochs}, negative={negative}, alpha={alpha}, sample={sample}, hs={hs}, cbow_mean={cbow_mean}")
    """
    ...
    if not os.path.exists('models'):
        os.makedirs('models')
    model_name = f"word2vec_skipgram_tk{method}_vs{vector_size}_w{window}_mc{min_count}_e{epochs}_neg{negative}_alpha{alpha}_hs{hs}_cbow{cbow_mean}.model"
...
if __name__ == "__main__":
    ...
    print(f"Word2Vec model trained and saved in 'models/word2vec_skipgram_tk{method}_vs{vector_size}_w{window}_mc{min_count}_e{epochs}_neg{negative}_alpha{alpha}_hs{hs}_cbow{cbow_mean}.model' using {method} tokenization.")
